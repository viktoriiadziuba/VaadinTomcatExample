...
public class Products extends VerticalLayout {
	...
	public Products() {
		...
		// WORKSPACE
		VerticalLayout workspace = new VerticalLayout(createProduct(),createProduct(), createProduct(), createProduct(), createProduct(), createProduct());
		...
		//buttonBar.add(new RouterLink("Suppliers", Suppliers.class));
	...
	private Component createProduct() {
		Span product1Label = new Span("Product");
		FlexLayout product = new FlexLayout(product1Label);
		product.addClassName("product");
		product.setAlignItems(Alignment.CENTER);
		product.setJustifyContentMode(JustifyContentMode.CENTER);
